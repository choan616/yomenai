...
test('요약 막대가 숙지한 표현을 밴드별로 나누고, 출제가 늘어도 안 짧아진다', async ({ page }) => {
  ...
  // **순번이 아니라 `data-band` 로 짝짓는다** (2026-09-26) — 순번 짝짓기는 한쪽 목록이
  // 달라지는 순간 거짓 통과·거짓 실패를 낸다. 무엇이 무엇의 색인지를 직접 묻는다
  ...
  // 숙지 0 인 밴드는 범례에도 안 낸다 (2026-09-26). 한 번 0% 로 냈다가 되돌렸다 —
  // 조각이 없는 범례 항목은 해독할 게 없고, 아래 표가 같은 밴드를 개수로 이미 말한다
  const bandsOf = (sel: string) =>
    page.evaluate(
      (s) => [...document.querySelectorAll(s)].map((d) => d.getAttribute('data-band')),
      sel,
    )
  expect(await bandsOf('.mix-dot')).toEqual(await bandsOf('.mix-seg'))
})
