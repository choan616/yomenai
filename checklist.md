...
- [ ] **음독맵·리포트에 안 잡힌다** — 두 화면이 `base.json` 만 보고 풀을 만든다.
      **담아 둔 밴드 4도 이미 같은 상태다** (이번에 생긴 문제가 아니다). 채점은 쌓이는데
      화면에서만 빠지므로, 고치려면 두 화면의 풀 구성을 세션과 맞춰야 한다.
      `pairRows` 가 `pairs.json` 에 없는 쌍을 버리는 것도 같이 봐야 한다 (의도된 규칙이라
      단독으로 뒤집지 않았다)
