...
- [ ] **학습 카드 UI 레이아웃 재구성** — 방향: 레이아웃 재구성(숫어 위치·입력법·피드백
  배치 재검토). 확정 요구사항:
  - 입력 폼 고정 — `<input>` 포커스 시 소프트 키보드가 뜨면 화면이 밀려 올라간다.
    입력 영역을 뷰포트 하단에 고정(키보드 위)해 레이아웃이 안 흔들리게
  - 그 외는 시안/레퍼런스 논의 후 착수. 현행 스크린샷 → 문제점 정리 → 새 레이아웃
    제안 → 승인 → 구현 순. 지금 개별 CSS 패치하면 재구성 때 두 번 일함
