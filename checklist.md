...
9. **기타 190건(18%)의 정체 · 청탁 관문 버그 · 「다른 읽기」** (사용자 지시) — 실사용
   ...
   표시 이름 「다른 읽기」를 줬다 — `MistakeType` 은 안 늘렸다(스키마 불변 조건).
   그 행은 정렬에 들고 「오답 유형별 다시보기」로 이어진다. 넘김은 답이 없는 것이라
