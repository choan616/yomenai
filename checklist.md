...
- [~] **12-D · 한국어 뜻 — stdict 대조에서 영어 gloss 번역으로 전환 (2026-09-07 방향 변경)**
  ...
  - **검수 진행** — 배치 18 반영 (2026-09-11, 커밋 `761a2c4`). 배치 18은 40건 중 인라인 수정 1(力投).
...
- [x] **`buildReport` 에 `frequent` 추가** — 오답 있는 읽기 카드를 오답 수 내림차순,
  상한 `BROWSE_N`(12). 동점은 id 순 (기기 병합 순서에 안 흔들리게)
  - **재대결과 달리 극복한 카드(`streak`)를 안 뺀다** — 출제가 아니라 노출이라
    최근에 맞힌 것도 다시 보는 게 이득이다. context-notes 같은 날 절
- [x] **리포트 「훑어보기」 섹션** (`BrowseSection`) — 처방 바로 뒤. 표제어 + 오답 횟수만
  접어 두고, 누르면 읽기 · 뜻 · 소리 듣기(TTS) · Tatoeba 예문 2개. 한 번에 하나만 펼친다
  - 이벤트를 안 쓴다. FSRS 도 안 건드린다 — 보기만 하는 화면이다
  - 예문(`loadExamples`)은 처음 펼칠 때 부른다. 리포트 첫 그림을 늦추지 않는다
- [x] **검증: `report.test.ts` 10→16 (정렬·뜻 카드 제외·이름 없는 숙어 제외·극복 카드 포함·
  동점 id 순·`BROWSE_N` 절단). `tests/e2e/report-browse.spec.ts` 신규 —
  세션 전부 오답 완주 → 리포트 → 펼침/접힘·`aria-expanded`·예문 로드·출제 요소 부재 확인.
  `npm test` 348 · `npm run e2e` 통과 · `tsc -b`/`oxlint` 클린 · 캡처로 레이아웃 대조**
