...
## 지금 여기 (2026-09-17 마지막 갱신)

이어받는 세션이 먼저 읽는 자리. **끝난 일은 아래 Phase 절에, 근거는 `context-notes.md` 에 있다.**
여기에는 *다음에 손댈 것* 만 적는다. 다 끝나면 이 절을 지우지 말고 내용을 갈아끼운다.

**상태** — `main` 깨끗함, 전부 푸시됨. 단위 454 · e2e 23 통과. 상용 배포 확인됨.
...
**2026-09-17 에 한 일 (전부 배포됨)**

1. 개발 로직 캔버스를 현재 상태로 다시 그림 + 옵시디언 참조본 지형으로 정렬 (선 관통 0)
2. **읽기 규칙 화면** — 음운 변화 8절(종성 대응 → 촉음 → 반탁 → 연성 → 장음, 연탁, 음독
   층위, 혼독) + 절마다 내 오답 기록을 얹은 색인. 예시 숙어는 전부 코퍼스 실재를 테스트로 확인
3. **탁음 갈래 분리** — 분류기가 버리던 정보를 `explainMistake` 가 돌려준다. 연탁·반탁·연성이
   각자 이름과 절을 갖는다. **이벤트 스키마는 안 건드렸다** (답으로 다시 매긴다, 소급 적용)
4. **하단 탭 셸** — 홈·리포트·찾기·설정 4탭. 홈 894 → 791px(넘침 0), `browseFrom`·`rulesFrom`
   같은 복귀 상태 제거. 세션·진단·다시보기는 탭바를 덮는다
5. **문구** — 「뜻은 아는데」 전제를 다섯 군데에서 걷어냄. 홈 태그라인은 사용자가 직접 쓴
   「일본어 한자, 당황하지 말자!」
6. **다시보기 규칙 배지** — 카드 머리에 「반탁 규칙」이 「2회 틀림」과 같은 줄로 붙고, 누르면
   그 절이 카드 안에서 펼쳐진다. 갈래 매기기는 `voicingByEvent` 로 올려 규칙 화면과 공유한다

**결정을 기다리는 것 (사용자 판단 필요)**

1. **규칙형 오답 12건** — 아래 「동형이독」 절의 「남은 구멍」. 連濁·促音·長音이 대체 읽기로
   존재해 조용히 정답 처리되는 경우. 빌드 필터 한 줄이면 막히지만 범위를 좁히는 일이라 물어야 한다
2. **탭 순서** — 「읽기로 찾기」가 실사용에서 제일 자주 여는 화면이 될 수 있다 (읽다 당황한
   자리에서 바로 여는 용도). 며칠 쓴 뒤 「제일 자주 여는 것」을 확인하고 조정한다.
   지금 추측으로 옮기지 않는다

**다시보기에 남은 것** — 규칙 절에서 「이 규칙으로 틀린 것만 다시보기」로 되짚는 반대 방향 연결
(아래 주차장). 배지가 절로 가는 길을 냈으니 그쪽은 절에서 카드로 오는 길이다.

**남은 UI 숙제** — 셸이 정해졌으니 미뤄 둔 둘을 이제 볼 수 있다. 「첫 안내를 모달 + 캐러셀로」,
「홈 레이아웃 본편」 (아래 「UI 개편 세션으로 미룬 것」 절).

**보류 중인 테스터 피드백** — 다시보기에 채점 버튼(「이해했어요/잘 모르겠어요」)을 붙이는 건
채점 없는 화면이라는 정체와 부딪혀 보류. 수준별 진단(N5/N4)은 기획에서 제외됨.
...
## 동기화 유실 — 자기 파일을 읽지도 않고 덮어썼다 (2026-09-13, 사용자 신고)
...
- [ ] **내비게이션 모델 (하단 탭)** — 지켜볼 신호는 `context-notes.md` 같은 날 절 참조.
  **이걸 먼저 정한다** — 모달·캐러셀보다 위다. 셸이 바뀌면 그 위의 것도 다시 짠다
- [ ] **홈 레이아웃 본편** — 지금 홈은 "테스트하기 편하게" 정리한 상태다 (사용자 판단)
- [ ] **사용 안내서를 설정과 같은 층으로** — 지금은 홈 하단 `home-nav` 의 셋째 칸이라
  리포트·음독 맵과 같은 무게로 읽힌다. 안내서는 "언제든 꺼내 보는 것" 이라 ⚙ 처럼
  상단에 있는 편이 맞다 (사용자 판단 2026-09-14). 상단에 아이콘 두 개를 둘지,
  ⚙ 안으로 넣을지는 내비게이션 모델을 정한 뒤에 결정한다
...
## 문법 노출 점검 — 축 A' · 구조 판정 유형 (2026-09-17, 착수)
...
## 청탁 미구분 — 「표시」 방식 (2026-09-18, 사용자 결정)
...
- [x] **3단계 · 미분류 3,521 을 청탁 미구분으로** (2026-09-18 완료). 나머지 둘은 아래.
  「3단계」로 묶어 부른 게 판단을 흐렸다 — 셋의 성격이 달라서 **미분류만 떼면 우선순위를
  안 건드린다.** `stringVoicing` 이 `defer` 를 내고 `fromStrings` 가 촉음·장음·한국음 간섭을
  다 거친 뒤 **마지막 `null` 자리에서만** 쓴다 — 아무 데서도 안 뺏는다
  - 검증: 전후 대조로 `unmarked` 7,928→11,449(+3,521) · 미분류 3,966→445(−3,521) ·
    `KO_INTERFERENCE`·`ONYOMI_CHOICE`·`rendaku`·`handaku`·`MIXED_READING` **전부 불변**
  - 남은 445 는 훈독 자리의 청탁(右手 みぎて ← みきて). 한국음은 훈독과 무관하므로
    미분류로 두는 게 맞다 — 테스트로 박았다
  - 깨진 테스트 다섯은 **논지를 살려** 고쳤다 (「분류가 안 된다」가 아니라 「연탁이 아니다」가
    그 테스트들의 논지였다. 3ef2a53 때는 이름이 없는 게 최선이었을 뿐)
  - 검증 실행: `npm test` **508** · `npx playwright test` **29 스펙** · `tsc -b`/`oxlint` 클린
...
1. **`ONYOMI_CHOICE`·`MIXED_READING` 감사** — 구조 판정 중 아직 미검증인 둘. 잣대와 도구가
   ...
   **탐침은 반드시 양방향으로 만든다** — 한쪽만 만들어 결론 낼 뻔한 게 오늘만 두 번이다

**결정이 있어야 움직이는 것**
2. 3단계(흩어진 7,336) · 새 오답 유형 — 위 두 항목 참조. 둘 다 진단 축이 움직인다

**사용자 몫**
3. 실기기 확인 다섯 건 (진단 체감 · 처방이 「다음에 뭘 할지」로 읽히는지 · iOS 키보드 올린 채
   카드 3~4장 · 배포판 정리 후 Drive 파일 · 예고가 궁금증인지 소음인지)

**손대면 안 되는 경계 (오늘 피로 배운 것)**
- **훈독 자리의 탁음 과잉은 진짜 연탁 과잉 적용이다** — `unmarked` 로 끌어오면 92e7381 이 깨진다
- **`-ノウ` 류를 연성으로 태그하지 않는다** — 음독 쌍 식별자가 바뀌어 지난 학습 기록이 다른
  쌍을 가리킨다
- **검증 장치를 먼저 의심한다** — 오늘 네 번 다 탐침·픽스처·음성 대조가 틀려서 결론이 틀렸다
