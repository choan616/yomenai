...
- [~] **3단계 · 흩어진 7,336 끌어오기 — 착수 전 사용자 결정이 필요하다.**
  미분류 3,521 · `KO_INTERFERENCE` 2,596 · `ONYOMI_CHOICE` 1,219 을 `unmarked` 로 끌어오는
  일이다. **할 일 목록이 아니라 결정 대기다** — 1·2단계와 달리 `fromStrings` 의 마지막 검사와
  `DECOMPOSED_PRIORITY` 를 건드려야 하고, 그러면 리포트 유형 분포와 처방 문턱이 움직인다.
  1·2단계는 「잘못 붙은 이름을 떼는」 쪽이라 파장이 좁았는데 이건 분포 자체가 바뀐다.
  묻지 않고 진행하지 않는다
