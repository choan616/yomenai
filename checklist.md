...
- [ ] **`大家(たいか)` 예문 셋이 전부 `大家族`** — `大|家族` 로 갈려 표제어 오른쪽 끝이
      형태소 한가운데를 자르는데, 「한쪽 끝만 중간이면 살린다」 예외(`論文|中` 의 `文中`)에
      걸리고 읽기 비교가 청음 정규화 뒤 부분 문자열이라 `だいかぞく→たいかそく` 에
      `たいか` 가 들어 있어 통과한다.
  - 범위는 **바깥에서 못 쟀다** — 27건·164건 두 번 다 오탐이 섞였다(`和菓子` 의 `菓子`,
    `一回`). 규칙을 빌더 안에 넣고 다시 돌려 빠지는 목록을 보는 것이 맞다

---

## 셸 화면이 밀려 올라가던 것 (2026-09-26, 사용자 보고)

증상 둘이 같은 원인이었다 — 하단 탭바가 손가락을 따라 올라가고, 자판이 내려간 뒤 탭바가
화면 한가운데 남고 아래가 비었다.

- [x] **원인** — 탭바가 `position: sticky; bottom: 0` 이라 **문서가 밀리면 같이 밀린다**.
      모바일에서 스크롤하는 것은 `.screen` 이고 문서는 안 움직이는 게 정상인데, iOS 가
      포커스된 입력을 보이려고 문서를 올리고 자판이 내려가도 안 되돌린다.
      단어장이 **자동 포커스 입력을 가진 유일한 탭 화면**이라 거기서 먼저 드러났다
- [x] **`--vvh` 가 아니었다** — 그 변수는 `.study`·`.diag` 만 쓴다. 첫 추측이 틀렸다
- [x] **높이도 `overflow` 도 안 건드린다** — `useViewportLock` 의 `body: hidden` + 높이 덮기는
      3분할 고정 골격 전용이라 셸에 걸면 화면 안 스크롤이 죽는다. **있으면 안 되는 스크롤을
      0 으로 되돌리는 것 하나만** 한다
- [x] **PC 는 제외** — 거기선 문서가 스크롤러다. CSS 와 같은 미디어 질의를 쓴다
- [x] **회귀 테스트** — 모바일 뷰포트에서 문서를 밀어 놓고 0 으로 돌아오는지 본다
      (크롬은 iOS 의 밀어 올림을 흉내 못 내므로 **계약**을 테스트한다)
- [x] **검증 실행** — `npm test` 710 · `playwright` 92 · tsc·oxlint·build 클린
