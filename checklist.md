...
- [ ] **`fix?: string`** — `MeaningVoteEvent` 에 선택 필드. 워크리스트 판정과 1:1
  - 검증: 옛 이벤트(필드 없음)가 그대로 읽힌다
- [ ] **숨은 진입** — 설정의 한 자리를 길게 눌러 「검수 모드」를 켠다. 기기별, localStorage
  - 검증: 켜기 전에는 설정에도 화면에도 흔적이 없다
- [ ] **검수 화면** (설정 아래 `Sub`) — 내가 만난 미검수 뜻을 훑는다
  - 검증: o / x+고침 이 `flag` 이벤트로 남고 마지막 것이 이긴다
- [ ] **도구** — `data/events/*.json` 의 `flag` 를 읽어 검수 TSV 에 판정을 채운다
  - 검증: 앱에서 찍은 o·x+fix 가 워크리스트에 그대로 뜨고 `apply` 까지 왕복한다
- [ ] **검증 실행** — `npm test` · `playwright` · tsc · oxlint · build
