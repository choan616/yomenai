...
### 갈래가 답의 모양을 따라가던 것 (2026-09-18, 사용자 지적 「寸法 는 반탁 아닌가」)
...
## 갈래는 정답 쪽 규칙이 정한다 (2026-09-18, 사용자 지적)

- [x] **`voicingOfExpected`** — 정답 조각이 들고 있는 변형을 먼저 본다. 없으면 음독은
  `unmarked`, 훈독은 어느 축이 달랐나에 맡긴다. 분해·문자열 경로가 같은 함수를 쓴다
  - 문제: 寸法 すんぽう ← すんぼう 가 **연탁**으로 갔다. 法는 `ぽう[handaku]` 자리인데
    답이 `ぼう` 라 `differs('rendaku')` 가 먼저 걸렸다 — **답의 모양이 갈래를 정했다**
  - 범위: 寸法만이 아니라 **307건 / 306개 숙어** (圧迫·安否·一杯 …). 전부 반탁 자리를
    탁음으로 쓴 오답이다
  - 검증: 전수 대조로 어긋남 **307 → 0**. 갈래가 언제나 정답 쪽 변형과 일치한다
  - 검증 실행: `npm test` **511** · `tsc -b`/`oxlint` 클린
