...
- [ ] **소개가 성취로 안 잡힌다** — 소개만 한 세션은 이벤트가 0이라 완주해도 앱이
  "아직 진단 안 함" 으로 본다. 「성취도는 깎이지 않는다」와 부딪힌다. 결정 대기
- [ ] **새 숙어 한 장에 탭이 두 번** — 「뜻은 알고 있었어요?」 + 「봤어요」.
  한 카드로 합치는 안 (뜻을 가린 채 묻고, 답하면 그 자리에서 펼침) 검토
