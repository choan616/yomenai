...
- [ ] **9-A · 홈 갈림길 + 해요체 말투** — 저비용, 먼저. 서로 독립이라 묶어 배포
  - 홈: 진단 전에는 "진입 진단"을 주 버튼, "세션 시작"을 보조로. 진단 후 반대
  - 해요체 전환 — 진단 프롬프트·결과, 세션 요약, "오늘의 발견", 빈/오답 안내 문구.
    `~해요 · ~네요 · ~볼까요? · 좋아요 · 힘내요`. 느낌표 남발·이모지·공허한 상찬·반말 금지.
    버튼 라벨(동사 단문)·`mistakeLabels` 분류 라벨·`lang="ja"` 콘텐츠는 손대지 않음
  - **결정**: 진단 리포트(앱의 얼굴)도 해요체로 갈지 — 권장 절제 유지
  - **검증**: e2e 셀렉터가 버튼 이름 의존 → 라벨 바뀌면 스펙 갱신, 5스펙 통과.
    tsc/oxlint/build 클린. 톤 육안 확인
- [ ] **9-B · 적응형 진단** — 90 → 25~35문항. 핵심 레버, core 로직
  - `pickDiagnostic` 은 상한만 넉넉히(밴드당 12) 뽑게. 조기 종료 판정은
    `Diagnostic.tsx` `advance` 에서 현재 밴드 누적으로:
    오답 3 도달 → 진단 전체 종료 / 8개 풀고 정답률 ≥ 75% → 다음 밴드 / 12개 상한
  - **결정**: 진단에서 "뜻 알았나요?" 인터스티셜 제거 — 권장 뺀다(지연 검수가 덮음).
    빼면 모드 배정 2단계 신호가 실제 세션까지 미뤄짐
  - **결정**: 임계값(오답 3 / 75% / 상한 12)은 초안 — 권장 그대로 시작, 로그 보고 조정
  - `diagnosticSummary` / `firstShaky` 는 유지 (seen 수만 작아짐)
  - **검증**: core 단위 테스트 — 밴드1 전패 → ≤ 6문항 종료, 전승 → 밴드2 진입,
    firstShaky 결과 불변. full-flow e2e 갱신. 진단 문항 수 실측
