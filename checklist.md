...
- [ ] **찾기 탭에 카메라 진입** — 새 탭·새 계통을 안 만든다
- [ ] **띠 미리보기 + 가운데 네모 + 화면 탭**으로 인식
  - 검증: 초록 네모와 실제로 자른 영역이 **같다** (가짜 카메라로 배율별 좌표 대조, 오차 ≤2px)
- [ ] **배율 1×/2×/3×**. 네모 크기는 **하나로 고정** (결정 11)
- [ ] **방향 단추를 화면에 안 둔다** (결정 10) — 기본 세로, 실패 시 재시도, 성공한 쪽 기억
- [ ] **결과는 찾기 화면으로** (결정 9) — 정확히 걸리면 입력란에 읽기를 채우고,
      아니면 후보를 기존 결과 카드로 나열한다
  - 검증: `Basket` 의 행 마크업을 공용으로 뽑아 후보 목록이 같은 모양을 쓴다
- [ ] **누른 티**를 준다 — 155ms 만에 끝나 안 그러면 아무 일도 안 난 것처럼 보인다
- [ ] **카메라 실패 경로** — 권한 거부·HTTPS 아님·기기 없음일 때 무엇을 할지 화면이 말한다
  - 검증: 권한을 막고 들어가도 화면이 안 죽고 사유가 뜬다

### 3단계 — 인식과 사전

- [ ] **조각을 줄여 넣는다.** 가로글은 높이, 세로글은 폭. 문턱을 넘을 때만
  - 검증: 큰 조각(760×528)에서 축소 없이 6/40 → 축소 후 37/40 이 재현된다
- [ ] **방향 — 기본 세로, 사전 적중 실패 시 반대로 한 번 더**
  - 검증: 세로 본문에 가로 모델이면 0/12, 재시도가 붙으면 회복된다
- [ ] **사전 조회** — 인식 결과 안의 어느 구간이든, **가장 긴 적중**이 이긴다
