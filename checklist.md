...
## 흔들림 표시 (2026-09-23, 사용자 지적 "라인이 어색하다")

- [x] **네 안을 실제 화면에 입혀 두 테마로 비교** — 朱 테두리 · 朱 사선 · 바탕 사선 · 투명도 40%
- [x] **바탕색 사선 채택** (사용자 선택) — 밴드 색을 남기고 색 채널을 안 건드린다
  - 흔들림의 朱 는 범례 글자와 판정 배지가 이미 들어 막대에서 빼도 경고가 남는다
- [x] **줄눈 투명도 50%** (사용자 요청) — `--hatch` 토큰, 테마 네 군데에 rgba 리터럴
- [x] **검사로 못 박음** — 밴드 색 유지 + `repeating-linear-gradient` + `box-shadow` 없음
  - 옛 테두리로 되돌려 **실패하는 것을 확인**했다
- [x] **비교용 `_variant.spec.ts` 제거** — 안 지워서 전체 e2e 에 2건이 딸려 들어가 있었다
- [x] **검증 실행** — `npm test` 651 · `playwright` 72 · tsc·oxlint·build 클린

---

## 밴드 행 정리 (2026-09-23, 사용자 요청)

- [x] **다섯 안을 375px 실제 숫자로 렌더해 비교** — B · C · D1 · D2 · D3
- [x] **D2(표 + 왼쪽 세로줄) 채택** (사용자 선택). 8줄 → 4줄 + 머리글
  - 수치가 **열로 맞아** 밴드끼리 비교가 된다. 이게 이 영역의 쓸모다
- [x] **표본 부족은 다른 색** (사용자 지시) — 무채 **점선**. 색과 결 두 채널이라 PLAN §7 유지
- [x] **`.sr-only` 로 판정 이름 유지** — 도형은 낭독이 안 된다. 유틸이 없어 같이 들였다
- [x] **빈 칸은 `0` 이 아니라 `—`** — 안 푼 것과 못 외운 것을 안 섞는다
- [x] **검사** — 머리글 · 열 정렬 좌표 일치 · 배지 0개 · sr-only 폭 · 큰 글자에서 한 줄
- [x] **검증 실행** — `npm test` 651 · `playwright` 72 · tsc·oxlint·build 클린

---

## 음독 맵과 밴드 표의 숫자 (2026-09-23, 사용자 보고)

- [x] **버그 — 음독 맵만 `studyPool` 을 안 썼다.** 분모 2,995 → 2,607쌍 (훈독 전용 388쌍, 13%)
  - 되돌려 **실패하는 것을 확인**했다 (2607 기대 ← 2995)
- [x] **단위 차이를 화면이 밝힌다** (사용자 선택 「둘 다 두되 서로를 밝힌다」)
  - 음독 맵 머리를 「한자 읽기 N쌍 중 K쌍 숙달」로 · 표현 수치와 잣대를 한 줄씩 병기
  - 리포트 진입 행을 「한자 읽기 K/N쌍 숙달」로
- [x] **「숙지」 판정을 한 자리로** — `isReadingStable`·`stableReadingCount` (`level.ts`)
  - 두 화면이 서로를 인용하므로 정의가 갈리면 안 된다
- [x] **도구 행은 계산 전에도 한 줄** — 줄이 늘면 묶음이 덜컥인다(`screen-cache.spec.ts`)
- [x] **검사** — 훈독 0%/100% 분모 일치 · 두 화면의 표현/쌍 수치 일치
- [x] **검증 실행** — `npm test` 651 · `playwright` 74 · tsc·oxlint·build 클린

---

## 카메라로 찾기 (2026-09-23, 기능화)

측정·결정 근거는 `context-notes.md` 2026-09-23 두 절에 있다.
시험 페이지(`public/ocr-test.html`)는 **마지막 단계에서 지운다.**
...
- [x] **찾기 탭에 카메라 진입** — 새 탭·새 계통을 안 만든다
- [x] **띠 미리보기 + 가운데 네모 + 화면 탭**으로 인식
  - 검증: 초록 네모와 실제로 자른 영역이 **같다** (가짜 카메라로 배율별 좌표 대조, 오차 ≤2px)
- [x] **배율 1×/2×/3×**. 네모 크기는 **하나로 고정** (결정 11)
- [x] **방향 단추를 화면에 안 둔다** (결정 10) — 기본 세로, 실패 시 재시도, 성공한 쪽 기억
- [x] **결과는 찾기 화면으로** (결정 9) — 정확히 걸리면 입력란에 읽기를 채우고,
      아니면 후보를 기존 결과 카드로 나열한다
  - 검증: `Basket` 의 행 마크업을 공용으로 뽑아 후보 목록이 같은 모양을 쓴다
- [x] **누른 티**를 준다 — 155ms 만에 끝나 안 그러면 아무 일도 안 난 것처럼 보인다
- [x] **카메라 실패 경로** — 권한 거부·HTTPS 아님·기기 없음일 때 무엇을 할지 화면이 말한다
  - 검증: 권한을 막고 들어가도 화면이 안 죽고 사유가 뜬다

### 3단계 — 인식과 사전

- [x] **조각을 줄여 넣는다.** 가로글은 높이, 세로글은 폭. 문턱을 넘을 때만
  - 검증: 큰 조각(760×528)에서 축소 없이 6/40 → 축소 후 37/40 이 재현된다
- [x] **방향 — 기본 세로, 사전 적중 실패 시 반대로 한 번 더**
  - 검증: 세로 본문에 가로 모델이면 0/12, 재시도가 붙으면 회복된다
- [x] **사전 조회** — 인식 결과 안의 어느 구간이든, **가장 긴 적중**이 이긴다
  - 검증: 꼬리에 잡소리가 붙은 입력에서 짧은 오적중(`和合`)을 안 고른다
- [x] **근사 매칭** — 같은 길이·한 자 차이를 **후보로만** 낸다. 단정하지 않는다
  - 검증: `軍朋` 을 넣으면 `軍艦` 이 후보에 뜨되 답으로 확정되지 않는다
- [x] **맞은 구간을 강조한다** — 원문 `4ら飲料缶の` 중 `飲料` 가 걸린 것인데 화면이
      그걸 안 알려 주면 사용자가 무엇을 찾았는지 모른다 (2026-09-23 실물 확인)
  - 검증: 앞뒤에 이웃 글자가 붙은 인식 결과에서 맞은 두 글자만 도드라진다
- [x] **결과는 기존 찾기 카드** — 담아 두기(`star`)가 그대로 동작한다. 새 이벤트 타입 없음
...
- [x] **네모가 정사각형 하나**로 (사용자 제안, 실측 확인). 방향마다 길쭉한 네모를 두려
      했는데 정사각형이 못지않았고, **뒤집을 때 다시 자를 필요가 없어진다**
- [x] **가로 PSM 7 → 6** — 정사각형은 이웃 줄이 1.4줄쯤 담겨 「한 줄」 모드가 실패한다
      (실측: 7 은 「抽」, 6 은 「爆弾に」). 단어만 꽉 찬 조각에서는 둘이 같았다
- [x] **npm 의존을 안 늘렸다** — 1단계 정적 자산을 주입한 script 로 부른다.
      소스에서 곧바로 import 하면 dev 서버가 500 을 낸다(public 은 변환 밖이라서)

---

## 찾기를 밴드 4까지 (2026-09-23, 사용자 보고)

- [x] **밴드 4 역인덱스** (`loadWideReadingIndex`) — 기본 16,959 + 밴드 4 85,418
- [x] **찾기 시작할 때 받는다** — 화면 여는 값으로 20MB 를 치르지 않는다. 받는 동안은
      기본 범위로 답하고, 오면 넓어진다
- [x] **카메라는 못 찾았을 때만 기다린다** — 맞히면 기다릴 일이 없다.
      기다린 뒤에는 **이미 읽어 둔 결과를 다시 맞추기만** 한다 (OCR 재실행 없음)
- [x] **밴드 4는 담기를 안 낸다** — 출제 범위 밖이라 담아도 안 나온다. 「학습 범위 밖」
- [x] **검사** — `search-wide.spec.ts`: いんうつ → 陰鬱 이 나오고 담기가 안 뜬다.
      기본 범위 숙어는 그대로 담긴다
- [x] **검증 실행** — `npm test` 668 · `playwright` 80 · tsc·oxlint·build 클린

### 뒤따라야 할 것

- [x] **밴드 4 뜻 번역** — 끝났다. `band4.json` 85,418개 중 85,417개에 뜻이 실려 있다
      (2026-09-24 확인. 7e01bbf 에서 반영된 뒤 이 줄이 안 닫혀 있었다)
- [ ] **표외자를 들일지** — `躊躇`·`邂逅` 는 밴드 4에도 없다 (임포트가 상용한자 밖 글자를
      버린다). 사전 재생산·밴드 재산정이 따라온다

---

## 담은 밴드 4 를 세션에 (2026-09-23, 사용자 요청)

- [x] **뜻이 없으면 교정 모드** — `assignMode` 에 `hasMeaning`. 밴드가 아니라 **뜻 유무**로
      가른다. 번역이 채워지면 저절로 원래 규칙을 탄다 (단위 검사 7건)
- [x] **담은 것만 세션 풀에** — 자동 출제 안 함. 담긴 id 중 기본 사전에 없는 게 있을 때만
      `band4.json` 을 받는다 (안 담았으면 20MB 를 안 받는다)
- [x] **담기를 되돌렸다** — 앞 절에서 막아 둔 것을, 이제 나오므로 다시 낸다
- [x] **검사** — 陰鬱 을 담으면 목록에 서고, 세션에 나오고, **뜻은 안 묻는다**
- [x] **검증 실행** — `npm test` 675 · `playwright` 81 · tsc·oxlint·build 클린
- [x] **번역 배치** — 106,802개 번역(플래그 1.3%). 사전 재빌드까지 끝났다
  - 빌더가 뜻을 `korean-class.json` 한 곳에서만 읽어 **밴드 4가 0개**로 나왔다.
    분류(밴드 0~3)와 뜻(전 밴드)은 범위가 다른 자료라 따로 읽게 고쳤다
  - 덤 — `glossEn`(화면 비표시 필드)이 번들로 새고 있었다. 빼서 base 5.63 → 4.61MB,
    band4 29.15 → 25.06MB. 프리캐시 여유 6.37 → 7.39MB

---

## 검수 경로를 담은 밴드 4까지 (2026-09-24, 사용자 판정)

- [x] **워크리스트가 담긴 밴드 4 를 얹는다** — 동기화 파일(`data/events/*.json`)에서 읽는다.
      `build-event-worklist` 관례 그대로. 마지막 `star` 가 이긴다
- [x] **어느 모드에서든 맨 앞에 싣는다** — 플래그가 없다고 표본에서 빠지면 무의미하다
- [x] **적용이 분류표에 자리를 만들어 받는다** — 전에는 `tally.missing` 으로 버렸다
- [x] **왕복 실측** — 陰鬱(x+fix) → manual·verified, 憂鬱(o) → verified, 「새 자리 2」.
      뺀 刹那 는 안 실린다. 시험 뒤 데이터 파일은 원래대로 되돌렸다
- [x] **검증 실행** — `npm test` 675 · tsc·oxlint·build 클린

### 남은 것

- [ ] **밴드 0~3 검수 진도** — 1,340/17,213(7.8%). 하던 대로 이어간다
- [ ] **표외자를 들일지** — 보류(2026-09-24). 임포트가 상용 밖을 버려 15,114개가
      빠진다(음독 약 9,700 · 빈도 있는 것 342). 다음에 볼 모양은 전면 도입이 아니라
      **표외자 항목을 밴드 4 로** — 찾기에는 뜨고 학습 풀은 안 건드린다. `context-notes.md` 참조
  - 먼저 잴 것: 표외자를 들이면 음독맵에 상용 밖 한자가 얼마나 올라오나

---

## 원격 검수 (2026-09-24, 사용자 요청)

- [x] **`fix?: string`** — `MeaningVoteEvent` 에 선택 필드. 워크리스트 판정과 1:1
  - 검증: 옛 이벤트(필드 없음)가 그대로 읽힌다
- [x] **숨은 진입** — 설정의 한 자리를 길게 눌러 「검수 모드」를 켠다. 기기별, localStorage
  - 검증: 켜기 전에는 설정에도 화면에도 흔적이 없다
- [x] **검수 화면** (설정 아래 `Sub`) — 내가 만난 미검수 뜻을 훑는다
  - 검증: o / x+고침 이 `flag` 이벤트로 남고 마지막 것이 이긴다
- [x] **도구** — `data/events/*.json` 의 `flag` 를 읽어 검수 TSV 에 판정을 채운다
  - 검증: 앱에서 찍은 o·x+fix 가 워크리스트에 그대로 뜨고 `apply` 까지 왕복한다
- [x] **검증 실행** — `npm test` 675 · `playwright` 83 · tsc·oxlint·build 클린

### 왕복 실측

- [x] 앱에서 찍은 판정이 **사전까지** 닿는다 — 明白(x+고침) → manual·verified,
      愛好(o) → verified, 愛国(~) → 미검수 유지, 취소한 悪化 는 안 건드린다
- [x] 워크리스트가 **앱에서 본 것**도 얹는다 (담은 것 + 판정한 것). 안 그러면 판정이
      실릴 줄이 없어 고리가 안 닫힌다
- [x] `?`(미기입)를 빈 칸으로 본다 — 처음엔 사람이 찍은 것으로 오인해 전부 건너뛰었다

### 남은 것

- [x] **손이 가는 칸 하나** — ①앱에서 검수분만 내보내기로 닫았다. 아래 절 참조

---

## 검수분만 내보내기 (2026-09-24, 「효율말고 실질적으로 어떤것이 바른방향인가」)

앱 판정을 **저장소에 추적되는 원본**으로 만든다. 손을 더는 게 아니라 자격을 맞추는 일이다
(`context-notes.md` 같은 날 절 — 기각한 ②Drive 직접 읽기 포함).

- [x] **`core/reviewExport.ts`** — `flag` 이벤트를 네 칸(`id`·`headword`·`verdict`·`fix`)
      TSV 로 좁힌다. 이벤트 어휘를 검수 TSV 어휘(`o`/`x`/`~`/`-`)로 여기서 한 번만 옮긴다
  - 검증: `deviceId`·눌렀을 때의 뜻이 파일에 0건, 칸 수 4 고정 — 실행함
- [x] **줄 순서 고정** — 표기순. 다시 내보내도 `git diff` 가 판정 변화만 보인다
  - 검증: 입력 순서를 뒤집어도 같은 문자열 (단위 테스트)
- [x] **화면의 「판정 내보내기」** — 이벤트를 다시 읽어 만든다. 목록에서 사라진 옛 판정도
      파일에는 남아야 한다
  - 검증: `tsc`·`oxlint`·e2e 83 통과
- [x] **`apply:app-review` 가 내보낸 파일을 읽는다** — 둘 다 있으면 내보낸 쪽이 이긴다
  - 검증: 아래 왕복
- [x] **`build:korean-meaning-worklist` 도 읽는다** — 판정이 실릴 줄을 만들어야 고리가 닫힌다
  - 검증: 내보낸 4건 중 런타임에 나가는 2건이 배치 맨 앞에 섰다
- [x] **「판정 복사」** — 클립보드로 바로 넘긴다. 폰에서 텍스트를 꺼내는 네 단계를 없앤다.
      파일 쪽은 남긴다 — 저장소에 커밋할 원본은 파일이라야 한다. 복사본엔 BOM 을 안 붙인다
  - 검증: e2e 에서 버튼 노출 + 파일 경로 왕복 (네 칸·deviceId 없음)
- [x] **파일 이름을 gitignore 예외에 맞췄다** — `korean-meaning-app-review.tsv` 는
      `!data/dict/*-review.tsv` 에 그대로 걸린다. 새 예외를 안 만든다

### 왕복 실측 — `data/events/` 0개로 닫힌다

- [x] 御辞儀(o) → `verified: true` · 御守(x+고침) → `source: manual`·`verified: true`
- [x] 抑々(~) 미검수 유지 · 되돌린 微風(`-`) 안 건드림
- [x] 시험 뒤 작업 파일·`korean-class.json` 을 원래대로 되돌렸다
- [x] **검증 실행** — `npm test` 684 · `playwright` 83 · tsc·oxlint·build 클린 (여유 7.39MB)

---

## 한자로 찾기 (2026-09-24, 「한자를 직접 붙여넣어도 검색이 되는지」)

- [x] **표기 인덱스** — `byHeadword`·`headKeys` 를 읽기 인덱스와 **같은 Map 에 안 넣는다**.
      화면이 결과를 읽기로 묶어 그리는데 한자가 묶음 제목에 들어가면 읽기처럼 보인다
  - 검증: 단위 테스트 7개 (정확 일치·앞부분·상한·정렬)
- [x] **`searchByHeadword` 는 읽기 묶음으로 돌려준다** — 한자로 찾았어도 알고 싶은 건 읽기라
      묶음 제목이 곧 답이다. 화면 렌더 경로를 그대로 쓴다
  - 검증: `生物` 가 せいぶつ·なまもの 두 묶음으로 갈린다
- [x] **입력에 한자가 섞이면 자동으로 표기 검색** — 별도 모드 전환을 안 둔다
- [x] **못 찾았을 때 문구를 나눈다** — 「…라는 표기가 사전에 없어요」
- [x] **검증 실행** — `npm test` 691 · `playwright` 84 · tsc·oxlint·build 클린 (여유 7.39MB)

### 남은 확인

- [x] **iOS 붙여넣기 확인** — 길게 누르면 「붙여넣기」가 뜬다. `inputMode="none"` 은
      시스템 자판만 막고 붙여넣기 메뉴는 막지 않는다 (실기 확인, 2026-09-24)
  - `隔世` → 정확 일치 `かくせい` 묶음이 앞, 앞부분 일치 `かくせいいでん`(隔世遺伝)이 뒤
- [ ] **안드로이드 붙여넣기** — 아직 확인 못 했다. 기기 생기면 같은 절차로 본다

---

## 넓힌 사전 (2026-09-25, 「학습용 사전 외의 범위에서 찾겠냐고 묻는 건 어떤가」)

기존 사전을 **안 건드린다**. 밴드 재계산·음독맵 재생성·검수 이전이 전부 없다.
되돌리려면 산출물만 지우면 된다.

### 2a. 산출물

- [x] **`tools/build-wide-dict.ts`** — 상용 밖 글자를 가진 표제어를 조회 전용으로 뽑는다.
      밴드·분류·음독 쌍·`readingKind` 를 **안 싣는다** — 학습에 안 쓰는 칸이 있으면 거짓말이다
  - 검증: 15,114개 · 2.22MB · `base`/`band4` 와 겹친 id 0
- [x] **한자 정보도 같이 싣는다** — 런타임 `kanji.json` 은 2,130자뿐이라 `轟` 의 한국음이
      없다. 없으면 찾기 결과에 「—」가 뜬다
  - 검증: 추가 2,698자. 그중 한국 한자음이 없는 글자 194자는 그대로 「—」다
- [x] **프리캐시에서 뺀다** — `globIgnores` + `check-precache.mjs` 의 `FORBIDDEN`,
      런타임 캐시는 `StaleWhileRevalidate` (band4 와 같은 자리)
  - 검증: 빌드 후 프리캐시 16개 유지, `dist/dict/wide.json` 은 정적 자산으로만 나간다
- [x] **표제어 하나에 읽기가 여럿이면 항목도 여럿** — `邂逅` 가 かいこう·わくらば 둘이다.
      표기로 찾으면 읽기 묶음이 갈려 나온다

### 2b. 폰트 두 번째 서브셋

- [x] **밴드 4 글자가 폴백으로 뜨고 있었다** — 찾기가 밴드 4 까지 열린 뒤(2026-09-23)
      폰트를 다시 안 만들어 `鬱`(憂鬱·陰鬱) 등 104자가 시스템 폰트로 렌더링됐다.
      한중일 통합이라 한국 자형이 뜰 수 있는 자리다. `--all` 옵션을 없애고 늘 넣는다
  - 검증: 본 서브셋 508 → 521KB (104자에 13KB). `fonts.test.ts` 에 밴드 4 커버리지 추가
- [x] **넓힌 사전 전용 서브셋** — 2,698자를 `unicode-range` 로 가른다. 본 서브셋과 겹친
      코드포인트 1개뿐이다
  - 검증: Regular 692KB · Bold 719KB, 글리프 2,698/2,698 전량
- [x] **`@font-face` 선언도 번들 밖에 둔다** — `unicode-range` 가 39KB 라 번들에 실으면
      넓힌 사전을 안 쓰는 사람도 매번 받는다. `public/fonts/wide.css` 로 내고 2c 에서 붙인다
  - `url()` 은 상대 경로다. 번들을 안 거쳐 Vite 의 base 치환이 없다
- [x] **프리캐시에서 뺀다** — `globIgnores` 에 `*-wide.woff2`·`wide.css`,
      런타임 캐시는 `CacheFirst` (고정 자산이라 ocr 과 같다)
  - 검증: 빌드 후 프리캐시 16개 유지

### 2c. 앱에서 묻고 불러오기

- [x] **0건일 때만 묻는다** — 학습 사전에서 나왔으면 그게 답이고, 밖엣것까지 섞으면 소음이다
- [x] **누르기 전에는 아무것도 안 받는다** — 사전 2.2MB + 전용 글꼴이 딸려 온다.
      한 번 받은 기기에서는 다시 안 묻는다 (localStorage, 기기별)
- [x] **사전과 폰트를 같이 붙인다** — `@font-face` 선언을 안 붙이면 `轟` 이 시스템
      폰트로 떨어져 한국 자형이 뜰 수 있다. woff2 는 `unicode-range` 가 알아서 받는다
- [x] **한국 한자음은 두 자료를 합쳐 본다** — `轟`(굉)은 넓힌 사전 쪽에만 있다
  - 검증: e2e 에서 `爆轟` → ばくごう · 폭굉 · detonation
- [x] **담기를 안 연다 (결정)** — 음독 분해가 없어 세션에 못 들어간다. 담아도 아무 일이
      안 일어나므로 별을 두면 거짓말이다. 「학습 중」 자리에 「범위 밖」을 적는 관례를 따른다.
      **그래서 음독맵 오염도 측정도 필요 없어졌다** — 조회 전용이라 학습 풀에 안 닿는다
- [x] **검증 실행** — `npm test` 693 · `playwright` 86 · tsc·oxlint·build 클린

### 뒤따라야 할 것

- [ ] **한국어 뜻** — 지금은 영어 gloss 를 그대로 보여 준다. 없는 것을 있는 척하지 않는
      쪽이라 당장은 이대로 둔다. 번역을 돌릴지는 실제로 써 보고 정한다
- [ ] **실기 확인** — 폰에서 `爆轟`·`躊躇` 를 찾고, 글자가 일본 자형으로 뜨는지

### 뒤따라야 할 것

- [ ] **한국어 뜻** — 지금은 영어 gloss 뿐이다. `build:korean-meaning` 을 넓힌 사전에
      돌릴지는 2c 에서 화면을 보고 정한다

---

## 단어장 (2026-09-25, 「단어장 기능도 만들고 싶다」)

「학습앱이 주는 것만 공부하는 것이 아니라 **별도로 공부하는 것까지 끌어올 수 있다면**」이
출발점이다. 넓힌 사전이 찾을 수 있게 했고, 단어장이 모아 둔다.

### 먼저 알게 된 것

- [x] **지금 「담아 둔 표현」은 단어장이 아니라 대기열이었다** — `!started` 로 걸러서
      세션에 한 번 나오면 목록에서 사라진다. 책에서 모은 말을 쌓아 두려면 정반대다.
      단어장의 1차 가치는 묶음도 메모도 아니고 **없어지지 않는 것**이다

### 1단계 — 전용 화면

- [x] **찾기에서 한 겹** (`Sub`) — 담기가 일어나는 곳이다. 탭은 안 늘린다
- [x] **담은 것 전부를 낸다** — 학습 시작한 것은 「학습 중 · ✗ N회」를 달고 남는다
- [x] **담은 게 없어도 진입로는 보인다** — 다 공부하고 나면 못 들어가는 일이 없게
  - 검증: e2e — 明白(학습 중) · 愛好(아직 안 나옴) 두 줄이 다 뜬다

### 2단계 — 묶음

- [x] **`StarEvent.list?`** — 선택 필드. 스키마·PK·스토어를 안 건드린다.
      묶음을 안 준 옛 이벤트는 「기본」으로 읽힌다
- [x] **옮기기는 새 `star` 이벤트다** — 마지막 것이 이긴다. append-only 그대로
- [x] **담을 때는 안 묻는다** — 「지금 담는 묶음」을 단어장에서 미리 정하고 찾기의 `+` 는
      그대로 한 번이다. 누를 때마다 고르게 하면 한 번 누를 일이 두 번이 된다
  - 검증: e2e — 새 묶음 만들기 → 옮기기 → 묶음이 둘로 갈린다

### 3단계 — 메모

- [x] **`StarEvent.memo?`** — 왜 담았는지. 밖에서 공부하다 담을 때 출처를 적는 자리다
  - 검증: e2e — 메모를 남기고 새로고침해도 남아 있다

### 검증 실행

- [x] `npm test` 698 · `playwright` 88 · tsc·oxlint·build 클린 (여유 7.39MB)
- [x] **`wordlist` 와 `starred` 가 늘 같은 집합** — 단위 테스트로 못 박았다.
      갈리면 화면과 세션이 서로 다른 걸 본다

---

## 들이기 — 넓힌 사전에서 담은 말을 학습 사전으로 (2026-09-25, 사용자 제안)

「확장보다 단어수집은 의미가 없다」는 판정에서 나왔다. 세션에 안 나올 것을 모아 두면
북마크일 뿐이다. **15,114개를 통째로 들이지 않고 담은 것만 올린다** — 통째로 들이면
음독 쌍이 2,532 → 4,794 가 되고 그 절반이 상용 밖 글자라 「숙지한 음독」이 재는 것이
달라진다 (실측).

- [x] **`adopt()`** — 읽기 분해와 음독 쌍을 런타임에 계산한다. 사전 재빌드가 없다
  - 검증: 단위 10개. `爆轟` → `爆:on:ばく`·`轟:on:ごう`, 밴드 4, `hasMeaning: false`
- [x] **가르는 기준은 상용한자가 아니라 분해 성공 여부** — 못 가르면 음독 맵·형제 대조·
      오답 분류가 하나도 안 붙어 들여도 학습이 안 된다
  - 실측: 들일 수 있는 것 9,915(음독 8,095 · 혼독 1,820) / 못 들이는 것 5,199
- [x] **그릴 때 미리 가른다** — 누른 뒤에 안 된다고 하면 늦다. 못 담는 줄엔 「읽기만」
  - 검증: e2e — `爆轟` 담기 뜸 / `逢引`(훈독만) 「읽기만」, 버튼 없음
- [x] **한자 자료를 먼저 합친다** — 런타임 `kanji.json` 에 `轟` 이 없어서, 안 합치면
      분해가 실패하고 후리가나·오답 분류가 **조용히** 안 붙는다
- [x] **단어장이 되짚어 온다** — 안 그러면 담은 줄이 단어장에서 조용히 사라진다
- [x] **세션에 실제로 나온다** — e2e 가 담기부터 카드까지 왕복한다
- [x] **검증 실행** — `npm test` 708 · `playwright` 90 · tsc·oxlint·build 클린

### 남은 것 / 알려진 구멍

- [x] **음독맵·리포트·홈 풀 정합** — 아래 절에서 맞췄다 (2026-09-26)
- [ ] **한국어 뜻** — 들인 말은 뜻이 없어 교정 모드로만 돈다. 번역을 돌릴지는 써 보고 정한다

---

## 풀 정합 (2026-09-26)

세션은 base + 담은 밴드 4 + 들인 말을 보는데 음독맵·리포트·홈은 `base.json` 만 봤다.
**담아 둔 밴드 4 를 공부해도 그 세 화면엔 안 잡혔다** — 채점은 쌓이는데 분모엔 없는 상태.
들인 말도 같은 구멍에 빠진다. 이번에 생긴 게 아니라 밴드 4 를 연 2026-09-23 부터 있던 것이다.

- [x] **`dict/pool.ts` 한 군데서 만든다** — 세션·음독맵·리포트·홈이 같은 함수를 쓴다.
      두 화면 주석에 「따로 세면 언젠가 갈라진다」가 이미 적혀 있었는데 정작 풀이 갈려 있었다
- [x] **`withRuntimePairs`** — 들인 말의 쌍은 `pairs.json` 에 없다(런타임 산물).
      **`pairRows` 의 「사전에 없는 pairId 는 제외한다」 규칙은 안 건드렸다** — 그건 자료가
      어긋났을 때의 방어선이라 두고, 아는 쪽이 완전한 Map 을 만들어 넘긴다
- [x] **홈 미리보기도 맞췄다** — 「세션이 보는 것과 같은 풀이라야 미리보기가 안 어긋난다」가
      주석에 있었는데 실제로는 어긋나 있었다
- [x] **담기 전에는 아무것도 더 안 받는다** — 담은 것이 전부 base 안이면 밴드 4(20MB)도
      넓힌 사전(2.2MB)도 안 부른다
- [x] **회귀 테스트** — 亜鈴(밴드 4)을 담아 공부하면 음독맵 분모가 **정확히 1** 는다.
      `鈴:on:れい` 가 밴드 0~3 어디에도 없어서 그 1 은 풀에 들어왔다는 뜻뿐이다
  - 검증: 고치기 전 코드로 돌리면 **실패**하는 것까지 확인했다
- [x] **검증 실행** — `npm test` 708 · `playwright` 91 · tsc·oxlint·build 클린

### 안 건드린 것

- `CameraFind` 는 OCR 대조용 사전이라 학습 풀이 아니다 (base+band4 그대로)
- `Diagnostic` 은 처음 한 번 수준을 재는 자리라 범위를 고정해 둔다

---

## 넓힌 사전 한국어 뜻 (2026-09-26)

영어 gloss 를 gemma4(Ollama)로 옮긴다 — 학습 사전의 뜻과 **같은 출처·같은 수준**이고
`source: 'llm'` · `verified: false` 로 실린다.

- [x] **`build:korean-meaning -- --wide`** — 넓힌 사전 항목은 `idioms.json` 에 아예 없어서
      `--band=4` 로도 안 잡혔다. 입력 경로를 하나 더 받는다
- [x] **산출물을 가른다** (`korean-meaning-wide.json`) — 한 파일에 같이 쓰면 다음에
      `--wide` 없이 돌렸을 때 그 몫이 조용히 빠진다. 캐시는 같이 쓴다 (id 가 안 겹친다)
- [x] **`build:wide-dict` 가 번역을 실어 준다** — 없으면 영어 gloss 만 실린다
- [x] **뜻이 붙으면 뜻 카드도 낸다** — `hasMeaning` 이 true 가 되어 확장 모드가 열린다.
      번역 전 빌드는 예전과 똑같이 돈다 (교정 모드 고정)
  - 검증: 단위 12개 (`wide.test.ts`)
- [x] **찾기 결과도 한국어로** — 있으면 한국어, 없으면 영어 gloss 그대로
- [x] **번역 배치 완료** — 15,114건 전량, 336ms/건(약 85분). 뜻이 빈 것 0개
  - 검증: `爆轟`→폭발 · `躊躇`→망설임, 주저함 · `邂逅`→우연한 만남 · `逢引`→은밀한 만남, 밀회
- [x] **사전에 실었다** — `wide.json` 2.22 → 3.38MB. 프리캐시 밖이라 예산은 그대로(여유 7.39MB)

### 뒤따라야 할 것

- [ ] **들인 말의 뜻 검수** — 미검수 15,114개가 늘었다. 검수 화면은 **만난 것만** 내므로
      실질 부담은 안 는다. `爆轟`→「폭발」처럼 느슨한 것이 있다(정확히는 폭굉)

---

## 카드가 끝나면 입력창을 비운다 (2026-09-26, 사용자 지적)

「두 읽기를 다 맞혔어요」 화면에 빈 입력창이 남아 무엇을 칠지 모르게 됐다.

- [x] **자리와 포커스는 그대로 두고 보이지만 않게 한다** — 언마운트하거나 `display:none`
      하면 iOS 에서 blur 되고, 다음 카드에서 제스처 밖 `focus()` 로는 키보드를 못 올린다
      (`KanaInput` 의 `locked` 주석이 이미 겪어 둔 함정). `opacity: 0` 로 자리를 남긴다
- [x] **「읽기 둘」 중간 단계는 그대로 둔다** — 바로 다시 칠 자리다
- [x] **오답일 때도 그대로 둔다** — 입력창 안 글자를 정답과 대조해 색으로 덮는 자리다
- [x] **회귀 테스트** — 다 맞힌 뒤 `.kana-input-shell.done` 이 붙고 **포커스가 입력창에
      남아 있는지**까지 본다. 포커스를 잃지 않는 것이 이 구현의 전부다

### 예문 결함은 아래 절에서 고쳤다

---

## 예문 — 연탁을 지우는 자리를 좁힌다 (2026-09-26, 사용자 지적)

`大家(たいか)` 의 예문 셋이 전부 `大家族` 이었다. 그 자리에 `大家` 라는 단어는 없다
(`大 + 家族`).

- [x] **원인** — `大(ダイ)|家族(カゾク)` 로 갈려 표제어 오른쪽 끝이 `家族` 한가운데를
      자르는데, 읽기 비교가 **연탁을 지운 뒤 부분 문자열**이라 `だいかぞく → たいかそく` 가
      `たいか` 를 품어 통과했다
- [x] **연탁 지우기를 두 자리로 좁혔다** — 끝이 형태소 경계에 **맞을 때**(`一|回`)와
      표제어가 형태소의 **꼬리일 때**(`雪合戦` 의 `合戦`). 연탁은 뒷요소의 첫소리에
      일어나므로 꼬리면 그 탁음이 표제어 자신의 변형이다. **앞부분은 아니다** —
      `大家族` 의 `だ` 는 `大` 의 제 음이다
- [x] **촉음 융합은 그대로 둔다** — `一(イチ)|回(カイ)` 의 `一回(いっかい)` 는 경계를
      안 자르는데도 필요하다. 그래서 정규화를 둘로 갈라 연탁만 선택적으로 만들었다
- [x] **단위 테스트 8개** — `大家族`·`裁判官` 은 막고 `雪合戦`·`和菓子`·`居酒屋`·`一回`·
      `論文中` 은 살린다
- [x] **재빌드 실측** — 예문 26,486 → 26,848. 빠진 68 · 들어온 74.
      `大家(たいか)` 는 「彼は英文学の大家だ。」로 바뀌었다
- [x] **검증 실행** — `npm test` 718 · `playwright` 92 · tsc·oxlint·build 클린

### 남은 흠

- [ ] **빠진 68건에 재배치분이 섞여 있다** — 규칙이 거른 것과, 새로 통과한 짧은 문장에
      밀려난 것이 같이 들어 있다. 갈라 세려면 두 빌드의 후보 단계를 비교해야 한다

### 되돌리기만으로는 떨렸다 (2026-09-26, 같은 날 후속 보고)

- [x] **매 `scroll` 에 0 으로 당기니 손가락과 싸웠다** — 되돌리기는 증상 처치였다.
      문서가 스크롤 가능한 것 자체가 원인이다
- [x] **모바일에서 `html, body { overflow: hidden }`** — 그건 **손가락 스크롤만** 막고
      프로그램 스크롤은 그대로다. 그래서 iOS 가 자판 때문에 미는 경우를 위해
      **자판이 오르내릴 때 한 번씩** 되돌리는 가드는 남기고, `scroll` 마다 되돌리는 쪽은 뺐다
- [x] **화면 안 스크롤은 안 죽는다** — `.screen` 이 자기 `overflow-y: auto` 를 따로 가진다
- [x] **회귀 테스트 둘** — `html`·`body` 가 hidden 이고 `.screen` 이 auto 인지(계약),
      밀린 뒤 `focusout` 에 0 으로 돌아오는지
- [x] **검증 실행** — `npm test` 718 · `playwright` 93 · tsc·oxlint·build 클린

---

## 숙자훈을 찾을 수 있게 한다 (2026-09-26, 「昨日도 없는 것은 수상하다」)

`昨日(きのう)` 이 `idioms.json` 에 nf10 으로 있는데 런타임 번들에 없었다. 읽기를 한자
단위로 못 갈라(`昨` 의 훈독은 자료에 없고 음독은 サク뿐) `build-runtime-dict` 가 버렸다 —
Phase 2 의 설계된 거부다.

- [x] **규모 실측** — 분해 실패로 빠진 표제어 **4,357** / 106,803. 그중 빈도 순위가 붙은 것
      **258**. `日本人`(nf01) `今年` `二人` `行方` `身体` `部屋` `今日` `一人` `為替`
      `相撲` `息子` `大人` … 소설을 읽으면 반드시 만나는 말들이다
- [x] **학습 대상으로는 안 들인다** — 음독 맵·형제 대조·연탁 오답 분류가 하나도 안 붙고,
      한국 한자음으로 유추하는 방식과도 안 맞는다 (「작일」로는 `きのう` 에 못 닿는다)
- [x] **조회 전용 번들** `lookup.json` (914KB, 4,357개) — 학습 풀을 만드는 쪽은 이 파일을
      **안 읽는다**. 세션·음독맵·리포트에 애초에 안 닿는다
- [x] **기본 색인에 같이 넣는다** — 밴드 4·넓힌 사전과 달리 물어보지 않는다. 914KB 라
      물어볼 무게가 아니고, 첫 실행부터 오프라인으로 찾아져야 한다 (프리캐시)
- [x] **담기는 「읽기만」으로 막는다** — 넓힌 사전에서 세운 기준과 같다
- [x] **한국어 뜻도 붙는다** — 밴드 0~3 은 이미 번역·검수를 돌린 것들이라 그대로 실린다
      (`昨日` → 어제, `今日` → 오늘)
- [x] **검증 실행** — `npm test` 718 · `playwright` 94 · tsc·oxlint·build 클린,
      프리캐시 17개(여유 7.39MB)

### 범례에 0% 를 냈다가 되돌렸다 (2026-09-26)

- [x] **막대와 범례의 기준을 갈랐다** — 막대는 0폭을 못 그리니 숙지가 있는 밴드만,
      범례는 글자라 **0% 도 말한다**. 표에 「출제 10 · 숙지 0」으로 있는 밴드가 범례에만
      없으면 그래프가 그 밴드를 빠뜨린 것처럼 보인다
- [x] **한 번도 안 만난 밴드는 양쪽 다 뺀다** — 안 푼 것을 0% 로 적으면 못 외운 것과
      섞인다 (표의 `NO_DATA` 와 같은 기준)
- [x] **색 대조 테스트를 순번이 아니라 `data-band` 로 짝짓게 고쳤다** — 범례가
      세그먼트보다 길어질 수 있어 순번 짝짓기는 거짓 통과·거짓 실패를 낸다
- [x] **회귀 테스트** — 범례의 밴드 목록이 표에서 출제가 있는 행과 **같은지** 본다
- [x] **검증 실행** — `npm test` 718 · `playwright` 94 · tsc·oxlint·build 클린

**되돌렸다 (같은 날, 사용자 판단).** 「퍼센트가 0%인데 범례로 나오는 것이 의미가 있나」.
맞는 지적이다 — 범례는 **막대를 해독하는 물건**인데 조각이 없는 항목은 해독할 게 없고,
바로 아래 표가 같은 밴드를 「출제 10 · 숙지 0」으로, 비율이 아니라 **개수로** 말한다.
0% 는 덜 정확한 중복이다.

「그래프에 그 밴드가 빠졌다」로 읽히는 건 맞지만, **숙지 막대에 숙지 0 인 밴드가 안 나오는
것은 결함이 아니라 사실 그대로**다. 문제를 잘못 읽고 고친 것이었다.

- [x] 범례를 다시 `stable > 0` 으로
- [x] **`data-band` 짝짓기는 남긴다** — 순번 짝짓기는 한쪽 목록이 달라지는 순간 거짓
      통과·거짓 실패를 낸다. 이번 되돌리기와 무관하게 나은 검사다
- [x] 회귀 테스트도 뒤집었다 — 범례 밴드 목록이 **세그먼트와 같은지** 본다
- [x] **검증 실행** — `npm test` 718 · `playwright` 94 · tsc·oxlint·build 클린

### 수준 지표를 밴드 0~3 으로 좁혔다 (2026-09-26, 사용자 판단)

「그래프나 숙지율에는 밴드 4를 노출하지 않는 게 나을 것 같다」. 밴드 4 는 출제 범위 밖이라
**담은 것만** 들어온다 — 곁가지를 진도에 더하면 총계가 무엇의 총계인지 흐려진다.

- [x] **숙지 총계·요약 막대** — 밴드 0~3 만 센다
- [x] **경계선·헤드라인도 같이 좁혔다** — 안 그러면 그래프는 밴드 4 를 빼면서 제목은
      「밴드 4가 경계예요」라고 말한다. 내가 골라 넣은 열 개가 흔들린다고 사다리의 경계가
      정해지면 그 지표의 뜻이 달라진다
- [x] **표에는 남긴다** — 출제·최근 정답률은 볼 값이 있다. 다만 **숙지 칸은 「범위 밖」**이다.
      숫자를 적으면 위 총계와 안 맞는다. `—`(안 푼 것)와 말이 달라야 해서 따로 쓴다
- [x] **캡션 한 줄** — 수준은 밴드 0~3 으로 잰다는 것과 그 이유
- [x] **단위 테스트 3개** — 밴드 4 가 흔들려도 경계로 안 잡고, 안정이어도 `solidThrough` 를
      못 끌어올리고, 표 행은 그대로 남는다
- [x] **검증 실행** — `npm test` 721 · `playwright` 94 · tsc·oxlint·build 클린

---

## 앱 검수 189건 반영 (2026-09-26)

앱에서 「판정 복사」로 넘어온 것을 사전까지 돌렸다. 어제 만든 고리의 **첫 실사용**이다.

- [x] **189건 중 186건 반영** — o 185 · x 1. 이미 사람이 찍은 칸 2개는 안 덮었다
      (앱 판정은 초벌, TSV 가 최종)
- [x] **`高見(こうけん)`** → 「의견, 뛰어난 의견」으로 `source: manual` · `verified: true`.
      같은 표기 `高見(たかみ)`(→ 높은 곳)는 따로 `o` 로 통과했다
- [x] **배포 번들 검수 완료 1,501 / 16,959** (1,323 → 1,501)

### 곁에서 찾은 회귀 — 밴드 4 뜻이 통째로 사라져 있었다

- [x] **원인은 내가 만든 것** — 어제 `--wide` 를 돌릴 때 `--band` 기본값이 3 이라
      `korean-meaning.json` 이 밴드 0~3 만으로 **다시 쓰였다**. 밴드 4 몫 85,418개가
      조용히 빠졌고 `build-runtime-dict` 가 그대로 실어 배포했다
- [x] **캐시가 원본이라 복구됐다** — `--band=4` 한 번으로 106,802건 되돌림 (LLM 호출 0)
- [x] **같은 함정을 막았다** — 산출물을 「이번 대상」으로 **다시 만들지 않고 얹는다**.
      대상에서 빠진 옛 항목은 남지만 그건 캐시에도 있는 것이라 거짓이 아니다.
      지우는 쪽이 잃는 게 크다
- [x] **검증 실행** — `npm test` 721 · `playwright` 94 · tsc·oxlint·build 클린,
      밴드 4 중 뜻 없는 것 85,418 → **1**
