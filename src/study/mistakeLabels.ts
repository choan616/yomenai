...
export const MISTAKE_ADVICE: Record<MistakeType, string> = {
  ...
  SOKUON:
    'ツ·チ·ク·キ 로 끝나는 음독 뒤에 カ·タ·ハ행(ハ는 ぱ행으로)이 오면 촉음이 붙어요 — 発達 はったつ, 学校 がっこう. 앞이 ク·キ면 サ행 앞에선 잘 안 붙어요(国際 こくさい).',
  CHOON:
    '장음(ー)이 붙는 자리인지 아닌지 헷갈리는 곳이에요. ょ·ゅ 뒤 う 를 빠뜨리거나(特徴 とくちょう), 반대로 없어야 할 자리에 붙이는(健保 けんぽ) 실수가 다 나요.',
  MIXED_READING:
